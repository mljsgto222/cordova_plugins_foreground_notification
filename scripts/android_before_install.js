...
module.exports = function(context){
	...
	console.log('setting android foreground notification...');
	...
	if(content.indexOf('com.mobishift.plugins.foregroundnotification.ForegroundNotification') === -1){
		content = content.replace('import org.apache.cordova.*;', 'import org.apache.cordova.*;\nimport com.mobishift.plugins.foregroundnotification.ForegroundNotification;');
		
		content = content.replace('super.onCreate(savedInstanceState);', 'super.onCreate(savedInstanceState);\nif(getIntent().hasExtra("foregroundUrl")){ForegroundNotification.setUrlPath(getIntent().getStringExtra("foregroundUrl"));}');
		if(content.indexOf('onNewIntent') === -1){
			content = content.replace('public void onCreate(Bundle savedInstanceState)', 'public void onNewIntent(android.content.Intent intent){\nsuper.onNewIntent(intent);\nif(intent.hasExtra("foregroundUrl")){ForegroundNotification.setUrlPath(intent.getStringExtra("foregroundUrl"));}}public void onCreate(Bundle savedInstanceState)')
		}else{
			content = content.replace('super.onNewIntent(intent);', 'super.onNewIntent(intent);\nif(intent.hasExtra("foregroundUrl")){ForegroundNotification.setUrlPath(intent.getStringExtra("foregroundUrl"));}');
		}
